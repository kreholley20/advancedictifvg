  ...
  return (
    <div className="grid gap-4 pb-16 lg:grid-cols-[220px_1fr]">
      <div className="space-y-2">
        ...
        <div className="mt-4">
          <div className="mb-2 text-xs font-medium uppercase tracking-wide text-stone-500">Recent entries</div>
            ...
            {(entries ?? []).slice(0, 30).map((e) => (
              <button
                ...
                className={`block w-full rounded-lg px-2 py-1.5 text-left text-sm transition-colors ${
                  e.date === selectedDate ? "bg-stone-800 text-stone-100" : "text-stone-400 hover:bg-stone-900"
                }`}
              >
                {fmtDate(e.date)}
              </button>
            ))}
            {entries && entries.length === 0 && <p className="text-xs text-stone-600">No entries yet.</p>}
          </div>
        </div>
      </div>
      ...
      {draft && (
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <h1 className="text-lg font-semibold text-stone-100">Journal — {fmtDate(selectedDate)}</h1>
            <Button onClick={handleSave}>{saved ? "Saved ✓" : "Save Entry"}</Button>
          </div>

          <Card>
            <h2 className="mb-3 text-sm font-semibold text-stone-200">Pre-Market</h2>
          ...
          <Card>
            <h2 className="mb-3 text-sm font-semibold text-stone-200">Post-Market Review</h2>
